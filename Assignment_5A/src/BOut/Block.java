...
{	
	...
	/**
	 * we need to know the previous hit mini ball id to check for multiple collisions
	 */
	...
	/**
	 *  Called when there is a collision with a miniball. It check whether it has been hit once or twice
	 *  if it is hit by two different miniball, it sets the block to inactive and sets the score in the Gamestate.
	 */
	public void collisionWithMiniBall(MiniBall mb){
		if(mb.canHitBlock()){
			if(miniBallID == -1){ // this block has not been hit by a miniball
				miniBallID = mb.getID();
				mb.increaseHitBlockCount();
			}
			if(miniBallID != mb.getID()){ // this block is hit second time by a miniball
				mb.increaseHitBlockCount();
				this.setActive(false);
				if(audio != null){
					audio.play(BlockDestroySound);
	...
	/**
	 * Sets the Mini Ball variable for when the class needs to reference for the collision
	 * @param mini ball a MiniBall to check for collision
	 */
	public void setMiniBall(MiniBall mb){
		this.miniBallID = mb.getID();
	}
