...
public class GameMenu extends GameObject {
  ...
  /**
   * File(names) of level xml files
   */
  private File[] levelFiles;
  
  /**
   * Index of current position in the file list
   */
  private int levelIndex;
  
  /**
   * File(names) of archive sav files
   */
  ...
  private void populateLevelChoices () {
    ...
    levelFiles = levelDirectory.listFiles(
      new FilenameFilter() {
  ...
  private enum Mode {
    Levels {
      ...
      String getChoiceString (GameMenu menu) {
        if (menu.levelFiles.length <= 0) { return ""; }
        return getDisplayFor(menu.levelFiles[menu.levelIndex].getName());
        } 
    ...
    Archives {
      ...
      String getChoiceString (GameMenu menu) {
        if (menu.archiveFiles.length <= 0) { return ""; }
        return getDisplayFor(menu.archiveFiles[menu.archiveIndex].getName());
        }
  ...
  private enum State
  {
    ...
    ChoosingLevel(Mode.Levels) {
      ...
      State gotPlay (GameMenu menu) {
        if (menu.levelFiles.length <= 0)
        {
          return this;  // no game to play
        }
        menu.parent.nextGameID = BreakOutEngine.BreakOutPlayGame;
        ((BreakOutEngine)menu.parent).nextLevel = menu.levelFiles[menu.levelIndex].getAbsolutePath();
        return Finish;
      }
    },
    ...
    ChoosingArchive(Mode.Archives) {
      ...
      State gotPlay (GameMenu menu) {
        if (menu.archiveFiles.length <= 0)
        {
          return this;  // no archive to restore: ignore key
        }
        menu.parent.nextGameID = BreakOutEngine.BreakOutRestore;
        ((BreakOutEngine)menu.parent).restoreFilename = menu.archiveFiles[menu.archiveIndex].getAbsolutePath();
        return Finish;
      }
    },
  ...
  /**
   * get level index back in range after increment/decrement
   */
  private void fixLevelIndex ()
  {
    int len = levelFiles.length;
    if (len <= 0)
    {
      levelIndex = 0;
    }
    ...
    {
      levelIndex = ((levelIndex % len) + len) % len;
    }
  }

  /**
   * get archive index back in range after increment/decrement
   */
  private void fixArchiveIndex ()
  {
    int len = archiveFiles.length;
    if (len <= 0)
    {
      archiveIndex = 0;
    }
    ...
    {
      archiveIndex = ((archiveIndex % len) + len) % len;
    }
  }
